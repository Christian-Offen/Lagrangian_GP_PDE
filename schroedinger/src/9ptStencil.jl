...
function DiscretiseLDensity(L)
	...
	# pay attention to ordering of dt,dx
	Ldxd(dt,dx,U0,U1) = dt*Ldx(dx,(U1+U0)/2,(U1-U0)/dt)

	function Ld(dt,dx,u,uup,uright,uupright)
...
function DELSolve(Ld,q0,q1)
    ...
    function DELObjective(q2)
        objective_pre(q1) = Ld(q0,q1)+Ld(q1,q2)
        return ForwardDiff.gradient(objective_pre,q1)
    end
