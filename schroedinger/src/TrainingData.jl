...
function CreateTrainingData(Lagrangian, rng; NoSamples=50,l = 1,T = .5, M = 10, N = 10, frequency_decayPhi = [-2,4],frequency_decayP = [-2,4],frequency_scalingP=0.2)
	...
	sample_freq(frequency_decay) = M*exp.(frequency_decay[1]*(0:no_freq-1).^frequency_decay[2]).*randn(rng, ComplexF64,(no_freq,NoSamples))

	random_freqPhi= sample_freq(frequency_decayPhi) 
    random_freqP = sample_freq(frequency_decayP);
